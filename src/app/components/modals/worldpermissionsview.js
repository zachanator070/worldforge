	...
	render(){
		...
		return (
			<Modal
				...
				onCancel={()=>{this.props.setShowWorldPermissionModal(false)}}
				footer={[]}
			>
				{
						...
						<div className='padding-md'>
							...
							<span className='margin-md'>
								<Select
									...
									showArrow={false}
									onChange={(value) => {this.setState({userToAdd: value})}}
								>
									{userAddOptions}
								</Select>
							</span>
							<Button disabled={this.state.userToAdd === null} onClick={() => {this.addReadUser(this.state.userToAdd)}} type='primary'>Add User</Button>
						</div>
						: null
				}
				<Table className='padding-md' pagination={{ pageSize: 25 }} columns={columns} dataSource={userData} rowKey={user => user._id}/>
				<div>
